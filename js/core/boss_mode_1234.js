...
export default class BossMode {
    constructor() {
        ...
        this.init();
        window.BossMode = BossMode; // Expose global
    }
            ...
            <div class="w-full h-full bg-cover bg-center flex flex-col items-center justify-center text-white relative" style="background-image: url('${this.wallpapers[this.wallpaperIndex]}'); box-shadow: inset 0 0 0 2000px rgba(0,0,0,0.3);">
                ...
                <div class="flex flex-col items-center gap-4 mt-20 animate-fade-in delay-500">
                    ...
                    <div class="flex gap-2 relative">
                        <input type="password" placeholder="PIN" class="bg-white/20 backdrop-blur border border-white/30 rounded px-3 py-2 outline-none text-white placeholder-gray-300 focus:bg-white/30 transition-all w-48 text-center" onkeydown="if(event.key==='Enter') BossMode.instance.login(this.value)">
                        <button class="bg-white/20 hover:bg-white/30 rounded px-3 border border-white/30 transition-all" onclick="BossMode.instance.login(this.previousElementSibling.value)"><i class="fas fa-arrow-right"></i></button>
                ...
                <div class="absolute bottom-8 right-8 flex gap-4 text-2xl">
                    ...
                    <i class="fas fa-power-off cursor-pointer hover:text-red-400 transition-colors" onclick="BossMode.instance.toggle(false)"></i>
                </div>
            </div>
        `;
    }
        ...
        this.overlay.innerHTML = `
            <div class="flex-1 relative overflow-hidden bg-cover bg-center flex flex-col transition-[background-image] duration-500" style="background-image: url('${this.wallpapers[this.wallpaperIndex]}')">
                ...
                <div class="absolute top-4 left-4 grid grid-cols-1 gap-4 w-20">
                    ...
                    ${this.createDesktopIcon('Q3 Report', 'fa-file-excel', 'text-green-500', "BossMode.instance.launchApp('excel')")}
                    ${this.createDesktopIcon('Meeting Notes', 'fa-file-word', 'text-blue-500', "BossMode.instance.launchApp('word')")}
                    ${this.createDesktopIcon('Chrome', 'fa-chrome', 'text-red-500', "BossMode.instance.launchApp('edge')")}
                </div>
        ...
        return `
            <div class="h-10 bg-[#f3f3f3]/90 backdrop-blur-xl border-t border-gray-300 flex items-center px-2 gap-2 z-[10001] select-none shadow-lg shrink-0 justify-between">
                ...
                <div class="flex items-center gap-1 h-full">
                     <div id="boss-start-btn" class="h-8 w-8 hover:bg-white/50 rounded flex items-center justify-center cursor-pointer transition-colors ${this.startMenuOpen ? 'bg-white/50' : ''}" onclick="BossMode.instance.toggleStartMenu()">
                    ...
                    ${apps.map(app => `
                        <div id="boss-switch-${app.id}" class="h-8 w-8 flex items-center justify-center rounded hover:bg-white/60 cursor-pointer transition-all relative group ${this.activeApp === app.id ? 'bg-white border-b-2 border-blue-500' : ''}" onclick="BossMode.instance.launchApp('${app.id}')">
                ...
                <div class="flex items-center gap-2 text-[11px] text-gray-700 h-full px-2">
                    ...
                    <div class="flex flex-col items-end leading-tight cursor-pointer hover:bg-white/50 px-2 py-0.5 rounded h-full justify-center" onclick="BossMode.instance.toggleNotification()">
                        <span>${time}</span>
                        <span>${date}</span>
                    </div>
                    <div class="w-1 h-full border-l border-gray-300 ml-1"></div>
                    <div class="w-2 h-full hover:bg-white/50 cursor-pointer" onclick="BossMode.instance.toggleNotification()"></div>
                </div>
            </div>
        `;
    }
        ...
        return `
            <div class="absolute bottom-11 left-2 w-72 bg-[#f2f2f2]/95 backdrop-blur-xl shadow-2xl rounded-t-lg border border-gray-300 flex flex-col z-50 animate-slide-up origin-bottom-left" style="height: 400px;">
                 <div class="p-4 flex-1">
                      ...
                      <div class="grid grid-cols-4 gap-2">
                           ${['excel','word','email','ppt'].map(app => `
                               <div class="flex flex-col items-center gap-1 hover:bg-white/50 p-2 rounded cursor-pointer" onclick="BossMode.instance.launchApp('${app}')">
                 ...
                 <div class="bg-[#e6e6e6] p-3 flex justify-between items-center border-t border-gray-300 rounded-b-lg">
                      ...
                      <i class="fas fa-power-off hover:bg-white/50 p-2 rounded cursor-pointer text-gray-600" onclick="BossMode.instance.toggle(false)"></i>
                 </div>
            </div>
        `;
    }
            ...
            <div class="absolute top-4 left-4 right-4 bottom-16 bg-white rounded-lg shadow-2xl flex flex-col overflow-hidden border border-gray-400 animate-pop-in">
                <div class="bg-[#217346] text-white flex items-center justify-between px-2 py-1 select-none h-8 shadow-sm relative z-30">
                    ...
                    <div class="flex gap-3 text-white/80">
                         ...
                         <i class="fas fa-times cursor-pointer hover:bg-red-500 px-2 transition-colors" onclick="BossMode.instance.closeWindow()"></i>
                    </div>
                </div>
                ...
                <div class="bg-[#f3f2f1] border-b border-[#e1dfdd] flex flex-col shadow-sm select-none z-20 relative">
                    <div class="bg-[#f3f2f1] px-2 py-1 flex gap-2 h-14 items-center border-b border-[#c8c6c4]">
                        <button class="flex flex-col items-center hover:bg-gray-200 p-1 rounded text-[10px]" onclick="BossMode.instance.generateExcelData()">
        ...
        return `
            <div class="absolute top-4 left-4 right-4 bottom-16 bg-white rounded-lg shadow-2xl flex flex-col overflow-hidden border border-gray-400 animate-pop-in">
                <div class="bg-[#b7472a] text-white flex items-center justify-between px-2 py-1 select-none h-8 shadow-sm">
                    ...
                    <div class="flex gap-3 text-white/80"><i class="fas fa-times cursor-pointer hover:bg-red-500 px-2" onclick="BossMode.instance.closeWindow()"></i></div>
                </div>
                <div class="bg-[#f3f2f1] px-2 py-1 flex gap-2 h-12 items-center border-b border-[#c8c6c4]">
                     ...
                     <button class="flex flex-col items-center hover:bg-gray-200 p-1 rounded text-[10px]" onclick="BossMode.instance.exportToPPT()">
    ...
    getWordContent() {
        return `
            <div class="absolute top-4 left-4 right-4 bottom-16 bg-white rounded-lg shadow-2xl flex flex-col overflow-hidden border border-gray-400 animate-pop-in">
                ...
                <div class="bg-[#f3f2f1] px-2 py-1 flex gap-2 h-14 items-center border-b border-[#c8c6c4]">
                     <button class="flex flex-col items-center hover:bg-gray-200 p-1 rounded text-[10px]" onclick="BossMode.instance.cycleDoc()">
                        <i class="fas fa-folder-open text-blue-800 text-xl"></i> <span>Open</span>
                     </button>
                     <button class="flex flex-col items-center hover:bg-gray-200 p-1 rounded text-[10px]" onclick="BossMode.instance.exportToDoc()">
                        <i class="fas fa-save text-blue-800 text-xl"></i> <span>Save</span>
                     </button>
                     
                     <div class="border-l border-gray-300 h-8 mx-1"></div>
                     
                     <button class="flex flex-col items-center hover:bg-gray-200 p-1 rounded text-[10px] ${this.wordStealthMode ? 'bg-gray-300 shadow-inner' : ''}" onclick="BossMode.instance.toggleWordStealth()">
                     ...
                     <div class="flex flex-col gap-0.5 justify-center">
                        ...
                        <select class="text-[10px] border border-gray-300 rounded p-0.5 outline-none" onchange="BossMode.instance.setStealthSource(this.value)">
                    ...
                    <div class="flex-1 flex flex-col bg-white">
                        ${selected ? `
                            ...
                            <div class="p-4 border-t border-gray-200 bg-[#f8f9fa] flex gap-2">
                                <button class="bg-[#0078d4] text-white px-4 py-1 rounded text-xs" onclick="BossMode.instance.replyEmail()"><i class="fas fa-reply"></i> Reply</button>
                            </div>
                        ` : ''}
                    </div>
                </div>
                    ...
                    <div class="flex-1 flex flex-col bg-white">
                        ...
                        <div class="p-4 border-t border-gray-200">
                            <div class="border border-gray-300 rounded p-2 flex gap-2">
                                ...
                                <button class="text-[#464775] p-1" onclick="BossMode.instance.sendChat()"><i class="fas fa-paper-plane"></i></button>
                            </div>
                        </div>
                    </div>
                </div>
